# This contains a wrapper to scale the state space of modular robot env (modified from open source code)
...
class ScaledStateWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.low1 = env.observation_space["base_point"].low
        self.high1 = env.observation_space["base_point"].high
        # self.observation_space = gym.spaces.Box(low=-np.ones(self.low.shape), high=np.ones(self.high.shape),
        #                                         dtype=np.float32)
        # elif isinstance(obs, Tuple):
        self.low2 = env.observation_space["robot"][1].low
        self.high2 = env.observation_space["robot"][1].high
        self.low3 = env.observation_space["target"].low
        self.high3 = env.observation_space["target"].high
        ...
        self.observation_space = gym.spaces.Dict(
            {
                "base_point": gym.spaces.Box(low=-np.ones(self.low1.shape), high=np.ones(self.high1.shape), shape=(3,),
                                             dtype=np.float32),
                "robot": gym.spaces.Tuple(
                    (gym.spaces.MultiDiscrete(np.zeros(10) + 2),
                     gym.spaces.Box(low=-np.ones(self.low2.shape), high=np.ones(self.high2.shape),
                                    shape=(10, 2), dtype=np.float_))),
                "target": gym.spaces.Box(low=-np.ones(self.low3.shape), high=np.ones(self.high3.shape),
                                         shape=(3,), dtype=np.float32),  # target's (x,y,z) in 3d space
            }
        )
        # else:
    ...
    def observation(self, obs):
        # if self.compound:
        state1 = obs["base_point"]
        discrete = obs["robot"][0]
        state2 = obs["robot"][1]
        state3 = obs["target"]
        ret = {"base_point": 2. * (state1 - self.low1) / (self.high1 - self.low1) - 1.,
               "robot": (discrete, 2. * (state2 - self.low2) / (self.high2 - self.low2) - 1.),
               "target": 2. * (state3 - self.low3) / (self.high3 - self.low3) - 1.}
...
class ScaledParameterisedActionWrapper(gym.ActionWrapper):
    """
    Changes the scale of the continuous action parameters to [-1,1]. Parameter space must be flattened!

    :return: Tuple((Discrete(n), Box(c_1), Box(c_2),...Box(c_n))
    """
...
class QPAMDPScaledParameterisedActionWrapper(gym.ActionWrapper):
    """
    Changes the scale of the continuous action parameters to [-1,1]. Parameter space not flattened in this case

    :return: Tuple((Discrete(n), Tuple((Box(c_1), Box(c_2),..., Box(c_n))))
    """
