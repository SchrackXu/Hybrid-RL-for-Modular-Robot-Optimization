...
from agents.basis import PolynomialBasis, SimpleBasis
...
class QPAMDPAgent(Agent):
    ...
    def __init__(self, observation_space, action_space,
        ...
        if self.phi0_size is None:
            assert self.phi0_func is None
        # raise error? Need to specify size of custom phi0_func
    ...
    def _set_parameters(self, parameters):
        ...
        for action in range(self.num_actions):
            ...
            self.parameter_weights[action] = \
                parameters[index: index + rows].reshape(self.parameter_weights[action].shape)
            index += rows
    ...
    def _rollout(self, env, update_actions=False, max_steps=None):
        ...
        if self.print_freq > 0 and self._total_episodes % self.print_freq == 0:
            if self.print_freq == 1:
                print("{0:5s} R: {1:.4f} r: {2:.4f}".format(str(self._total_episodes),
                                                            self.R/self._total_episodes, sum(rewards)))
            else:
                # print("{0:5s} R: {1:.4f}".format(str(self._total_episodes), self.R/self._total_episodes))
                returns = np.array(env.get_episode_rewards())
                print('{0:5s} R:{1:.5f} P(S):{2:.4f}'.format(str(self._total_episodes),
    ...
    def _enac_gradient(self, env, max_steps=None):  # , phi0_func=None, phi0_size=None):
        ...
        """
        if self.phi0_size is None:
            assert self.phi0_func is None  # raise error? Need to specify size of custom phi0_fun
