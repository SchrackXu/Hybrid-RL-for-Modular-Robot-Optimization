...
class PADDPGAgent(Agent):
    ...
    def __init__(self,
        ...
        if replay_memory is None:
            self.replay_memory = MemoryNStepReturns(replay_memory_size, observation_space.shape,
                                                    (self.num_actions+self.action_parameter_size,), next_actions=False,
                                                    n_step_returns=self.n_step_returns)
        else:
            self.replay_memory = replay_memory
        self.actor = actor_class(self.observation_space.shape[0], self.num_actions,
        ...
        self.critic = critic_class(self.observation_space.shape[0], self.num_actions,
                                   self.action_parameter_size, **critic_kwargs).to(device)
        self.critic_target = critic_class(self.observation_space.shape[0], self.num_actions,
                                          self.action_parameter_size, **critic_kwargs).to(device)
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            if self.np_random.uniform() < self.epsilon:
                ...
                offsets = np.array([self.action_parameter_sizes[i]
                                    for i in range(self.num_actions)], dtype=int).cumsum()
    ...
    def _optimize_td_loss(self):
        ...
        with torch.no_grad():
            ...
            off_policy_next_val = self.critic_target.forward(next_states,
                                                             pred_next_actions, pred_next_action_parameters)
